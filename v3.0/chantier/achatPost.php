...
<?php
  ...
  $sql2 = mysqli_query($db, "SELECT CHA_NumDevis FROM Acheter WHERE ACH_NumAchat ='$numAchat'");
...
if (isset($_POST["delete"])) {
    $query = "DELETE FROM Acheter WHERE ACH_NumAchat='$numAchat'";
    $sql = mysqli_query($db, $query);
    $errr = mysqli_error($db);

    if ($sql) {
        echo '<div id="good">
  		        <label>Achat supprimé avec succès</label>
  		        </div>';
    } else {
        echo '<div id="bad">
  		         <label>L\'achat n\'a pas pu être supprimé</label>
  		         </div>';
    }
}
