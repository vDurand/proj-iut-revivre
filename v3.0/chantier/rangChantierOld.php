...
?>
    ...
    <div id="corps">
    <div id="labelT">
        <form method="post" action="rangChantierOld.php" name="EditClient" style="display: inline-block; float: left; margin-left: 10px;">
            <table>
                <tr>
                    <td>
        ...
        <label style="margin-right: 207px;">Liste des Chantiers Archivés</label>
    </div>
    <br>
        ...
        <table class="sortable" cellpadding="5">
            ...
            if ($_POST["submit"] == '#') {
                ...
                if ($_POST["submit"] == 'Rechercher') {
                    $nom = $_POST["Nom"] . '%';
                    $reponse = mysqli_query($db, "SELECT * FROM ChantierMax Join TypeEtat ON IdMax=TYE_Id WHERE IdMax>3 AND upper(CHA_Intitule) like upper('$nom') ORDER BY CHA_DateDebut DESC");
                } else {
                    $reponse = mysqli_query($db, "SELECT * FROM ChantierMax Join TypeEtat ON IdMax=TYE_Id WHERE $sorter like '$alpha' AND IdMax>3 ORDER BY $sorter");
                }
            }
