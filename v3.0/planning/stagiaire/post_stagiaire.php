<?php
	$pageTitle = "Envoi de planning stagiaire";
	$pwd='../../';
...
<div id="corps">
  <?php
  	if(isset($_POST['Date']) && isset($_POST['Tableau']) && isset($_POST["Modify"]) && isset($_POST['typePL']) && isset($_POST['TableauLogo']) && isset($_POST['couleur']))
  	{
  		$date=$_POST["Date"];
  		$typePL = $_POST['typePL'];
		$query = mysqli_query($db, "SELECT count(distinct ASSOC_date) as 'result' FROM pl_association WHERE ASSOC_date=date('".$date."') AND PL_id = 3 ORDER BY ASSOC_date DESC;");
        ...
        if($_POST["Modify"] == 'true')
		{
            if(mysqli_query($db, 'SET autocommit=0;') && mysqli_query($db, 'START TRANSACTION;'))
            {
                ...
                $query = mysqli_query($db, 'DELETE FROM pl_association WHERE ASSOC_date=date("'.$date.'") AND PL_id = 3;');
                if($query)
                {
                    ...
                    {
                        $query = mysqli_query($db, 'DELETE FROM pl_proprietees WHERE ASSOC_date=date("'.$date.'") AND PL_id = 3;');
                        if($query)
                        {
                            ...
                            {
                                $query = mysqli_query($db, 'DELETE FROM logo_association WHERE ASSOC_date=date("'.$date.'") AND PL_id = 3;');
                                if($query)
                                {
