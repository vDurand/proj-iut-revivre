		...
		if($_POST["Modify"] == 'true')
		{
            if(mysqli_query($db, 'SET autocommit=0;') && mysqli_query($db, 'START TRANSACTION;'))
            {
                ...
                if($query)
                {
                    ...
                    {
                        $query = mysqli_query($db, 'DELETE FROM pl_proprietees WHERE ASSOC_date=date("'.$date.'") AND PL_id = 1;');
                        if($query)
                        {
                            ...
                            {
                                $query = mysqli_query($db, 'DELETE FROM logo_association WHERE ASSOC_date=date("'.$date.'") AND PL_id = 1;');
                                if($query)
                                {
            ...
            if(mysqli_query($db, 'SET autocommit=0;') && mysqli_query($db, 'START TRANSACTION;'))
            {
                ...
                if(!$query)
                {
                    ...
                    echo '<div id="bad">
                          ...
                          </div>';//erreur lors de l'enregistrement
                }
                else
                {
                    $queryString = "INSERT INTO pl_proprietees(PL_id,ASSOC_date,ASSOC_Couleur) VALUES (".$typePL.",date('".$date."'),'".$couleur."')";
