...
?>
    <div id="corps">
        ...
        $queryMembre = "SELECT SAL_NumSalarie FROM salaries WHERE PER_Num = $num";
        $sqlMembre = mysqli_query($db, $queryMembre);
        $donnees = mysqli_fetch_assoc($sqlMembre);
        ...
        if ($sql) {
            ...
            if ($sql2) {
                ...
                if($sql3){
                    if($dateSortie != null){
                        $query4 = "UPDATE membre SET ME_DateSortie = '$dateSortie' WHERE SAL_NumSalarie = $numSal";
                ...
                <label>Membre modifie avec succes</label>
                        ...
                        $reponse = mysqli_query($db, "SELECT * FROM Salaries sal JOIN Personnes pe ON sal.PER_Num=pe.PER_Num JOIN Type ty ON sal.TYP_Id=ty.TYP_Id JOIN Fonction USING (FCT_Id) JOIN membre using(SAL_NumSalarie) WHERE sal.PER_Num='$num' ORDER BY PER_Nom");
                            ...
                            <td>
                                <table cellpadding="10" class="detailClients">
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align: left; width: 300px;"><?php if($donnees['ME_DateSortie'] == null && $donnees['ME_DateSortie'] == "0000-00-00") echo "Aucune date de sortie"; else echo DATE_FORMAT(date_create($donnees['ME_DateSortie']),"d/m/Y"); ?></td>
                                    </tr>
                                </table>
                            </td>
                        </table>
              ...
              <span>
                ...
                <?php
                        ...
                        echo '<div id="bad">
                  <label>ERREUR n°4 : Le membre n a pas pu etre modifie</label>
                  </div>';
                    }
                } else {
                    echo '<div id="bad">
              <label>ERREUR n°3 : Le membre n a pas pu etre modifie3</label>
              </div>';
                }
                mysqli_free_result($reponse);
            } else {
                echo '<div id="bad">
              <label>ERREUR n°2 : Le membre n a pas pu etre modifie2</label>
              </div>';
            }
        } else {
            echo '<div id="bad">
              <label>ERREUR n°1 : Le membre n a pas pu etre modifie1</label>
              </div>';
        }
        ?>
    </div>
