        ...
        if(mysqli_query($db, 'SET autocommit=0;') && mysqli_query($db, 'START TRANSACTION;'))
        {
            if($typeAction == "edit")
            {
                ...
                for($x=0; $x<sizeof($tableau); $x++)
                {
                    ...
                    if($tableau[$x][2]==1)
                    {
                        $query = mysqli_query($db, "INSERT INTO typesortie(TYS_ID, TYS_Libelle) VALUES (".($tableau[$x][0]).",'".addslashes($tableau[$x][1])."');");
                        if(!$query)
                        {
						echo "INSERT INTO typesortie(TYS_ID, TYS_Libelle) VALUES (".($tableau[$x][0]).",upper('".$tableau[$x][1]."'));";
                            $erreur = true;
                        }
                    }
                    else if($tableau[$x][2]==0)
                    {
                        $query = mysqli_query($db, "UPDATE typesortie SET TYS_Libelle='".addslashes($tableau[$x][1])."' WHERE TYS_ID=".($tableau[$x][0]).";");
                        if(!$query)
                        {
							$v=$tableau[$x][0];
							$w=$tableau[$x][1];
