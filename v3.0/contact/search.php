...
?>
    ...
    $queryCliEnt = mysqli_query($db, "SELECT * FROM Clients WHERE CLI_Nom IS NOT NULL AND CLI_Nom like '%".$search."%' ORDER BY CLI_Nom");
    ...
    // Employes clients
    $queryCliEmp = mysqli_query($db, "SELECT * FROM Clients cll JOIN EmployerClient cl ON cll.CLI_NumClient=cl.CLI_NumClient JOIN Personnes pe ON cl.PER_Num=pe.PER_Num WHERE CLI_Nom IS NOT NULL AND (PER_Nom like '%".$search."%' OR PER_Prenom like '%".$search."%' OR CLI_Nom like '%".$search."%') ORDER BY PER_Nom");
    while ($cliEmp = mysqli_fetch_assoc($queryCliEmp)) {
        ...
        <form method="get" action="client/detailEmployeC.php" name="detailEmploye">
            ...
            <tr onclick="javascript:submitViewDetail('<?php echo $cliEmp['PER_Num']; ?>', 'detailEmploye')"
                style="font-size: 14;">
                <td><?php echo formatUP($cliEmp['CLI_Nom']); ?></td>
    ...
    $queryCliPart = mysqli_query($db, "SELECT * FROM Clients cl JOIN EmployerClient em ON cl.CLI_NumClient=em.CLI_NumClient JOIN Personnes pe ON em.PER_Num=pe.PER_Num WHERE CLI_Nom IS NULL AND (PER_Nom like '%".$search."%' OR PER_Prenom like '%".$search."%') ORDER BY PER_Nom");
    ...
    // Fournisseurs
    $queryFourn = mysqli_query($db, "SELECT * FROM Fournisseurs WHERE FOU_Nom like '%".$search."%' ORDER BY FOU_Nom");
    ...
    $queryFouEmp = mysqli_query($db, "SELECT * FROM Fournisseurs fo JOIN EmployerFourn em ON fo.FOU_NumFournisseur=em.FOU_NumFournisseur JOIN Personnes pe ON em.PER_Num=pe.PER_Num WHERE PER_Nom like '%".$search."%' OR PER_Prenom like '%".$search."%' OR FOU_Nom like '%".$search."%' ORDER BY PER_Nom");
    while ($fouEmp = mysqli_fetch_assoc($queryFouEmp)) {
        ...
        <form method="get" action="fournisseur/detailEmployeF.php" name="detailEmpF">
            ...
            <tr onclick="javascript:submitViewDetail('<?php echo $fouEmp['PER_Num']; ?>', 'detailEmpF');"
                style="font-size: 14;">
                <td><?php echo formatUP($fouEmp['FOU_Nom']); ?></td>
    ...
    // membres revivre
    $reponse = mysqli_query($db, "SELECT * FROM Salaries cl JOIN Personnes pe ON cl.PER_Num=pe.PER_Num JOIN Type ty ON cl.TYP_Id=ty.TYP_Id WHERE PER_Nom like '%".$search."%' OR PER_Prenom like '%".$search."%' ORDER BY PER_Nom");
    ...
    // referents
    $queryRef = mysqli_query($db, "SELECT * FROM Referents cl JOIN Personnes pe ON cl.PER_Num=pe.PER_Num JOIN Prescripteurs USING(PRE_Id) WHERE PER_Nom like '%".$search."%' OR PER_Prenom like '%".$search."%' ORDER BY PER_Nom");
